    ...
    def plot(self):
        if self.companySelector.company_validator():
            ...
            try:
                ...
                #randy here i'm setting up the gui elements rn 
                #note that you will probably have to use tkinter 

                #randy here, I was messing around with the gui
                #still not functional but working at it
                #currently rewrote some of it as it conflicted with the code but for now I'll push 
                window = Tk()
                btn=Button(window, text="Type in year", fg='blue')
                btn.place(x=80, y=100)
                lbl=Label(window, text= "Select year", fg='Orange', font=("helvetica", 16))
                lbl(x=60, y=50)
                txtfld=Entry(window, text="This is Entry Widget", bd=5)
                txtfld.place(x=80, y=50)
                window.title('By year')
